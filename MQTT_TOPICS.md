...
All log entries from all systems. Consumed by `Utility: Logging`, written to daily JSONL file. CRITICAL entries are auto-forwarded to `highland/event/notify`.

| | |
|--|--|
| **Publisher** | Any flow or system (Node-RED, Z2M, Z-Wave JS, HA, etc.) |
| **Consumers** | Utility: Logging (exclusive writer to disk) |
| **Retained** | No |
| **QoS** | 2 — guaranteed delivery; log entries must not be silently dropped |
...
`system` values: `"node_red"` | `"ha"` | `"z2m"` | `"zwave_js"` | `"watchdog"`

> **Essential service:** `Utility: Logging` does not use the Initializer Latch and does not depend on the `initializers` context store. It inlines all required helpers directly so it remains functional even if Initializers fails.
...
**`highland/status/initializers/ready`**

Non-retained signal published by `Utility: Initializers` when the `initializers` context store has been fully populated. Consumed by flows using the `Initializer Latch` subflow that started before Initializers completed.

| | |
|--|--|
| **Publisher** | Utility: Initializers |
| **Consumers** | Initializer Latch subflow (indirectly, via flag polling) |
| **Retained** | **No** — must be non-retained. A retained message from a previous session would falsely signal readiness before the current session's utilities are loaded. The `initializers` memory store resetting on restart makes stale state structurally impossible. |
...
> This topic is a session-scoped signal, not a persistent state. The authoritative readiness check is `global.get('initializers.ready', 'initializers')` — this topic is only needed for flows that started before Initializers completed and are waiting for the signal.

---

**`highland/status/config/loaded`** ← RETAINED

Published by `Utility: Config Loader` after each successful load cycle. Retained so a restarting consumer can immediately see when config was last loaded and whether any errors occurred.

| | |
|--|--|
| **Publisher** | Utility: Config Loader |
| **Consumers** | Monitoring dashboards, diagnostic tooling |
| **Retained** | Yes |

```json
{
  "loaded": ["device_registry", "flow_registry", "notifications", "thresholds", "healthchecks", "secrets"],
  "errors": [],
  "scope": "all",
...
`scope` values: `"all"` | `"{config_name}"` (for selective reload via `highland/command/config/reload/{name}`)

---

**`highland/status/mqtt/probe`**

Non-retained round-trip probe used by the Node-RED / MQTT Edge health check. Node-RED publishes to this topic and subscribes to it simultaneously — receipt proves the broker is routing messages.

| | |
|--|--|
| **Publisher** | Utility: Health Checks (Node-RED / MQTT Edge group) |
| **Consumers** | Utility: Health Checks (same flow, MQTT in node) |
| **Retained** | No |
| **QoS** | 0 — delivery guarantee not needed for a health probe |

```json
{}
```

> This topic exists solely as a broker round-trip marker. Its payload is intentionally empty. Nothing outside the Health Checks flow should subscribe to or publish to this topic.
...
| Date | Change |
|------|--------|
| 2026-03-19 | Added infrastructure topics from build session: `highland/status/initializers/ready`, `highland/status/config/loaded`, `highland/status/mqtt/probe`. Updated `highland/event/log` entry with QoS 2, publisher table, and essential service note. |
